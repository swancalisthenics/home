...
function switchPage(pageId) {
    ...
    if(pageId === 'blog') {
        ...
        document.getElementById('legal-page').classList.add('active-page');
        window.scrollTo({top: 0});
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    filterButtons.forEach(button => {
        button.addEventListener("click", () => {
            // 1. Aktive Klasse bei den Buttons wechseln
            filterButtons.forEach(btn => btn.classList.remove("active"));
            button.classList.add("active");

            // 2. Filter-Wert auslesen (all, ernaehrung, uebungen)
            const filterValue = button.getAttribute("data-filter");

            // 3. Blog-Karten filtern
            blogCards.forEach(card => {
                ...
                if (filterValue === "all" || cardCategory === filterValue) {
                    ...
                    // Optional: Sanftes Einblenden per CSS-Klasse oder Inline-Style
